...
def wav2mfcc(file_path, max_len=47):
    wave, sr = librosa.load(file_path, mono=True,
                            sr=None)  # monaural / stereophonic sound (un seul channel/+ieurs channels)
    # sr=44.1khz

    mfcc = librosa.feature.mfcc(wave, sr, n_mfcc=40)  # sampling rate / default hop value 512
...
def save_data_to_array(path, max_len=47):
...
def get_test():
    # Get available labels
    labels, indices, _ = get_labels(DATA_PATH1)
